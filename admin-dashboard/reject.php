...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Update the events.status to 'rejected' using prepared statements
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                ...
                // No rows affected might mean ID was valid but status was already 'rejected'
